...
namespace CS2_SDK//开发者工具库(防止和基础函数冲突)
{
	...
	namespace CS2_Offsets//CS2固定偏移量 (游戏更新时需要同时更新 https://github.com/a2x/cs2-dumper.git)
	{
		...
		uintptr_t dwLocalPlayerController = 0x1A0B668;
		uintptr_t dwLocalPlayerPawn = 0x1823128;
		uintptr_t dwEntityList = 0x19BBCC8;
		uintptr_t dwViewAngles = 0x1A27C50;
		uintptr_t dwViewMatrix = 0x1A1DDE0;
		uintptr_t m_hPlayerPawn = 0x80C;
		uintptr_t m_iTeamNum = 0x3E3;
		uintptr_t m_ArmorValue = 0x23F4;
		uintptr_t m_iHealth = 0x344;
		uintptr_t m_iIDEntIndex = 0x1450;
		uintptr_t m_fFlags = 0x3EC;
		uintptr_t m_iShotsFired = 0x23D4;
		uintptr_t m_vecVelocity = 0x400;
		uintptr_t m_bSpotted = 0x1B40;//m_entitySpottedState + m_bSpotted
		uintptr_t m_bIsScoped = 0x23C0;
		uintptr_t m_pClippingWeapon = 0x1398;
		uintptr_t m_pGameSceneNode = 0x328;
		uintptr_t m_vecOrigin = 0x88;
		uintptr_t m_aimPunchCache = 0x1598;
		uintptr_t m_vecViewOffset = 0xCA8;
		...
		uintptr_t m_iItemDefinitionIndex = 0x134A;//m_AttributeManager + m_Item + m_iItemDefinitionIndex
		uintptr_t m_angEyeAngles = 0x1430;
	}
	...
	void ReLoad(BOOL Timeout = false) noexcept//刷新CS2进程地址和模块地址和有效实体
	{
		if (System::Sleep_Tick<class CLASS_CS2_SDK_Timeout_Reload>(1000) || Timeout)
		{
			...
			if (System::Sleep_Tick<class CLASS_CS2_SDK_Offsets_Timeout_Reload>(5000) || Timeout)//自动更新偏移量延迟 (减少流量使用)
			{
				...
				if (URL_OFFSETS.StoreMem("https://github.com/danaja416/pastehookcloud/Offsets.ofs?raw=true"))//自动更新偏移量 Github更新有十分钟延迟 中国用户需要挂梯子
				{
